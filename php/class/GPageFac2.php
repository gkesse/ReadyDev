<?php
class GPageFac2 extends GModule {
    //===============================================
    private $m_id = 0;
    ...
    private $m_defaultPagePath = "cache_default_page.xml";
    //===============================================
    ...
    public function setPage($_obj) {
        ...
        $this->m_siteName = $_obj->m_siteName;
    ...
    //===============================================
    public function createSite($_parent) {
        if($this->m_siteName == "") {
            $this->addError("Le site de la page est obligatoire.");
            return false;
        }
        $lPath = sprintf("%s/%s", $_parent, $this->m_siteName);
        if(!file_exists($lPath)) {
    ...
    public function createPage($_parent) {
        ...
        $lPath = sprintf("%s/%s%s", $_parent, $this->m_siteName, $this->m_path);
        if($this->isFile()) {
            if(!file_exists($lPath)) {
                $lDir = dirname($lPath);
                if(!file_exists($lDir)) {
    ...
    public function onStoreDefaultPage($_data) {
        ...
        $lFile = new GFile();
        $lFile->saveData($this->m_cacheDataPath, $this->m_defaultPagePath, $lData);
    ...
    public function onLoadDefaultPage($_data) {
        $lFile = new GFile();
        $lData = $lFile->loadData($this->m_cacheDataPath, $this->m_defaultPagePath);
        if(!$lFile->hasErrors()) {
    ...
    public function onSavePage($_data) {
        ...
        if(!$this->hasErrors()) {
            $lPath = GPath::create($this->m_cachePagePath);
            if(!$this->createSite($lPath)) return false;
    ...
    public function onSavePageFile($_data) {
        ...
        if(!$this->hasErrors()) {
            ...
            $lFile = new GFile();
            $lFile->saveData($this->m_cachePagePath, $lPath, $this->m_content);
            $this->addLogs($lFile->getLogs());
        }
        return !$this->hasErrors();
    }
