...
class GErrorUi extends GObjectUi {
    ...
    public function run() {
        ...
        $lTitle         = $this->m_app->getData("errors", "title");
        $lIntro         = $this->m_app->getData("errors", "intro");
        $lModalId       = $this->m_app->getData("errors", "modal_id");
        $lBodyId        = $this->m_app->getData("errors", "body_id");
        $lFormId        = $this->m_app->getData("errors", "form_id");
        $lMsgId         = $this->m_app->getData("errors", "msg_id");
        $lModule        = $this->m_app->getData("errors", "module");
        $lKeypressCB    = $this->m_app->getData("errors", "keypress_cb");
        $lCloseCB       = $this->m_app->getData("errors", "close_cb");
        ...
        for($i = 0; $i < $lCount; $i++) {
            $lCategory  = $this->m_app->getData3("errors", "category", $i);
            $lModel     = $this->m_app->getData3("errors", "model", $i);
            $lId        = $this->m_app->getData3("errors", "id", $i);
        ...
        for($i = 0; $i < $lCount; $i++) {
            ...
            $lModel     = $this->m_app->getData3("errors", "model", $i);
            $lId        = $this->m_app->getData3("errors", "id", $i);
            $lModule    = $this->m_app->getData3("errors", "module", $i);
            $lCallback  = $this->m_app->getData3("errors", "callback", $i);
            $lPicto     = $this->m_app->getData3("errors", "picto", $i);
            $lText      = $this->m_app->getData3("errors", "text", $i);
